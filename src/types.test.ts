import { Array, BigInt, Bool, Date, Float, Int, Regex, Tuple } from './types';
...
describe('Int()', () => {
    it.each`
        ...
        ${'1000000'} | ${1_000_000}
        ${'Imogen'}  | ${null}
    `('should be able to convert strings to numbers "$input" → "$output"', ({ input, output }) => {
...
describe('BigInt()', () => {
    it.each`
        ...
        ${'1000.50'} | ${null}
        ${'Imogen'}  | ${null}
    `('should be able to convert strings to BigInt "$input" → "$output"', ({ input, output }) => {
...
describe('Float()', () => {
    it.each`
        ...
        ${'100.50'} | ${100.5}
        ${'Imogen'} | ${null}
    `('should be able to convert strings to Float "$input" → "$output"', ({ input, output }) => {
...
describe('Float.DP()', () => {
    ...
    `('should be able to convert strings to Float.DP(𝑥) "$input" → "$output"', ({ input, options, output }) => {
...
describe('Bool()', () => {
    it.each`
        ...
        ${'no'}     | ${false}
        ${'Imogen'} | ${null}
    `('should be able to convert strings to Boolean "$input" → "$output"', ({ input, output }) => {
...
describe('Date()', () => {
    it.each`
        ...
        ${'Imogen'}     | ${null}
    `('should be able to convert strings to Date "$input" → "$output"', ({ input, output }) => {
...
describe('Array()', () => {
    ...
    `('should be able to convert strings to Array(𝑥) "$input" → "$output"', ({ input, options, output }) => {
...
describe('Tuple()', () => {
    ...
    `('should be able to convert strings to Tuple(…𝑥) "$input" → "$output"', ({ input, options, output }) => {
...
describe('Regex()', () => {
    ...
    `('should be able to convert strings to Regex "$input" → "$output"', ({ input, options, output }) => {
