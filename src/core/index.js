...
export function create(name, ...middleware) {
    ...
    const constructor = class extends prototype {
        constructor() {
            super();
            this[queue] = new Set();
            this[state] = 'normal';
        ...
        async render(mergeProps = {}) {
            ...
            const scheduledTask = new Promise(async resolve => {
                ...
                if (prevScheduledTask && !this[queue].has(prevScheduledTask)) {
    ...
    middlewares.set(constructor, middleware);
    return tag;
}
...
export function alias(name, newName) {
    ...
    const middleware = middlewares.get(constructor);
    return create(newName, ...middleware);
}
